...
import { Starbucks } from '../data/themes.json';

import styled from 'styled-components';
import { PageAuthor, PageContainer, PageImage, PageTitle } from '../styles';
import Share from '../components/share';

const files = Object.entries(Starbucks.props);

const StarbucksComponent = () => {
  const [file] = useState(files);

  return (
    <Layout>
      <SEO 
        title={`${Starbucks.name} - PaintChart`}
        description={`${Starbucks.description}`}
        image={`https://paintchart.app/img/starbucks/bg.png`} 
      />
      <PageContainer>
        <PageImage 
          src={`/${Starbucks.backgroundUrl}`} 
          alt={`Starbucks theme thumbnail`} 
        />
        <TitleShareContainer>
				  <PageTitle>{`${Starbucks.name}`}</PageTitle>
					<Share
						url={`https://paintchart.app/starbucks`}
...
export default memo(StarbucksComponent);
